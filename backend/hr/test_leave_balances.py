...
class LeaveBalanceTests(HrBase):
    def allocate(self, days=10, year=2026, carried=0):
        response = self.client.post(reverse('leaveallowance-list'), {'employee': self.emp_a.pk, 'year': year, 'leave_type': 'annual', 'entitled_days': days, 'carried_days': carried, 'note': 'Reviewed allocation'}, format='json')
        self.assertEqual(response.status_code, 201, response.data)
        return LeaveAllowance.objects.get(pk=response.data['id'])

    def request_leave(self, start='2026-09-10', end='2026-09-12'):
        response = self.client.post(reverse('leaverequest-list'), {'employee': self.emp_a.pk, 'start_date': start, 'end_date': end, 'leave_type': 'annual'}, format='json')
        self.assertEqual(response.status_code, 201, response.data)
    ...
    def test_approval_consumes_and_cancellation_restores(self):
        ...
        self.assertEqual(self.client.post(reverse('leaverequest-approve', args=[pk])).status_code, 200)
        self.assertEqual(balance(allowance)['remaining_days'], 4)
        self.assertEqual(self.client.post(reverse('leaverequest-cancel', args=[pk]), {'reason': 'Cancelled trip'}, format='json').status_code, 200)
        self.assertEqual(balance(allowance)['remaining_days'], 7)
    ...
    def test_cannot_reduce_below_usage_or_delete_allocation(self):
        ...
        self.assertEqual(self.client.patch(url, {'entitled_days': 1, 'note': 'Reduction'}, format='json').status_code, 400)
        self.assertEqual(self.client.patch(url, {'entitled_days': 8, 'note': 'Reviewed adjustment'}, format='json').status_code, 200)
        self.assertEqual(self.client.delete(url).status_code, 405)

    def test_validation_and_company_isolation(self):
        allowance = self.allocate()
        payload = {'employee': self.emp_a.pk, 'year': 2026, 'leave_type': 'annual', 'entitled_days': 5, 'note': 'duplicate'}
        self.assertEqual(self.client.post(reverse('leaveallowance-list'), payload, format='json').status_code, 400)
        payload.update(employee=self.emp_b.pk, year=2027)
        self.assertEqual(self.client.post(reverse('leaveallowance-list'), payload, format='json').status_code, 400)
        url = reverse('leaveallowance-detail', args=[allowance.pk])
        self.assertEqual(self.client.patch(url, {'entitled_days': -1, 'note': 'Invalid'}, format='json').status_code, 400)
        self.assertEqual(self.client.patch(url, {'entitled_days': 20}, format='json').status_code, 400)
        other = LeaveAllowance.objects.create(company=self.company_b, employee=self.emp_b, year=2026, leave_type='annual', entitled_days=10, note='Other company')
        self.assertEqual(self.client.get(reverse('leaveallowance-detail', args=[other.pk])).status_code, 404)
