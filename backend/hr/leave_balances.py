...
def usage(employee_id, company_id, year, leave_type, status=LeaveRequest.APPROVED, exclude=None):
    requests = LeaveRequest.objects.filter(
        ...
        status=status, start_date__lte=date(year, 12, 31), end_date__gte=date(year, 1, 1),
    ).exclude(pk=exclude)
    return sum((days_in_year(start, end, year) for start, end in requests.values_list("start_date", "end_date")), Decimal(0))


def balance(allowance):
    used = usage(allowance.employee_id, allowance.company_id, allowance.year, allowance.leave_type)
    pending = usage(allowance.employee_id, allowance.company_id, allowance.year, allowance.leave_type, LeaveRequest.PENDING)
    total = allowance.entitled_days + allowance.carried_days
    return {"total_days": total, "used_days": used, "pending_days": pending, "remaining_days": total - used}
...
def check_leave_balance(leave):
    ...
    for year in range(leave.start_date.year, leave.end_date.year + 1):
        allowance = LeaveAllowance.objects.filter(company_id=leave.company_id, employee_id=leave.employee_id, year=year, leave_type=leave.leave_type).first()
