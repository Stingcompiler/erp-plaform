...
class LeavePolicyTests(HrBase):
    def policy(self, **overrides):
        data = {
            "leave_type": "annual", "annual_days": "24", "minimum_service_months": 0,
            "prorate_first_year": True, "carryover_limit": "5", **overrides,
        }
    ...
    def generate(self, year):
        return self.client.post(reverse("leaveaccrualpolicy-generate"), {"year": year}, format="json")
    ...
    def test_carryover_is_capped_and_only_previous_remaining_is_carried(self):
        ...
        LeaveAllowance.objects.create(
            company=self.company_a, employee=self.emp_a, year=2025, leave_type="annual",
            entitled_days=20, carried_days=0, note="Previous year",
        )
        self.client.post(reverse("leaverequest-list"), {
            "employee": self.emp_a.pk, "leave_type": "annual", "start_date": "2025-12-01", "end_date": "2025-12-10",
        }, format="json")
    ...
    def test_policy_validation_generation_is_scoped_and_archiving_keeps_history(self):
        policy = self.policy()
        duplicate = self.client.post(reverse("leaveaccrualpolicy-list"), {
            "leave_type": "annual", "annual_days": "10", "carryover_limit": "0",
        }, format="json")
        self.assertEqual(duplicate.status_code, 400)
        invalid = self.client.patch(
            reverse("leaveaccrualpolicy-detail", args=[policy["id"]]), {"annual_days": "-1"}, format="json"
        )
        self.assertEqual(invalid.status_code, 400)
        self.assertEqual(self.generate("bad").status_code, 400)
        self.assertEqual(self.client.delete(reverse("leaveaccrualpolicy-detail", args=[policy["id"]])).status_code, 204)
