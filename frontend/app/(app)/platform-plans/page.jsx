...
const EMPTY = { code: "", name: "", description: "", currency: "USD", price: "", billing_cycle: "monthly", modules: "sales, inventory", users: "", branches: "", warehouses: "", ...EMPTY_COPY };
...
function PlanCopyEditor({ plan, onSaved, t }) {
  const [open, setOpen] = useState(false); const [value, setValue] = useState(() => ({ ...EMPTY_COPY, ...Object.fromEntries(COPY_KEYS.map((key) => [key, plan[key] ?? EMPTY_COPY[key]])) })); const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  const save = async (event) => { event.preventDefault(); setSaving(true); setError(""); try { await platformSubscriptions.updatePlan(plan.id, copyFrom(value)); setOpen(false); await onSaved(); } catch { setError(t("platformPlans.saveError")); } finally { setSaving(false); } };
  if (!open) return <Button type="button" variant="outline" onClick={() => setOpen(true)}>{t("platformPlans.editCopy")}</Button>;
  return <form onSubmit={save} className="mt-4 space-y-4 rounded-control border border-line bg-paper p-4">{error && <p role="alert" className="text-sm text-danger">{error}</p>}<CopyFields value={value} onChange={setValue} t={t} /><div className="flex gap-2"><Button type="submit" disabled={saving}>{saving ? t("common.saving") : t("common.save")}</Button><Button type="button" variant="outline" onClick={() => setOpen(false)}>{t("common.cancel")}</Button></div></form>;
}
