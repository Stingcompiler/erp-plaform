...
function PlanEditor({ plan, onSaved, t }) {
  ...
  const initial = () => ({
    ...
    users: latest?.limits?.users ?? "", branches: latest?.limits?.branches ?? "", warehouses: latest?.limits?.warehouses ?? "",
  });
  ...
  const save = async (event) => {
    ...
    try {
      ...
      if (versionChanged()) {
        await platformSubscriptions.createPlanVersion({ plan: plan.id, version: (latest?.version || 0) + 1, currency: value.currency, price: Number(value.price), billing_cycle: value.billing_cycle, modules: normalizeModules(value.modules), limits: limitsFrom(value), published_at: new Date().toISOString() });
      }
